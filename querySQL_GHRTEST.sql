...
-- CAU 5:
CREATE FUNCTION FUN_THONGTIN_HOPDONG_NHANVIEN
()
RETURNS TABLE
AS
RETURN SELECT HD.MANV "Mã nhân viên",
              NV.HOVATEN "Họ tên",
              HD.TENLOAIHOPDONG "Hợp đồng",
              HD.NGAYBATDAU "Ngày bắt đầu",
              HD.NGAYKETTHUC "Ngày kết thúc"
       FROM dbo.HOPDONG HD
           FULL JOIN dbo.NHANVIEN NV
               ON NV.MANV = HD.MANV
       WHERE HD.NGAYBATDAU =
       (
           SELECT MAX(NGAYBATDAU) FROM dbo.HOPDONG WHERE dbo.HOPDONG.MANV = NV.MANV
       )
             AND NV.MANV = NV.MANV;
GO
...
AS
BEGIN
    SELECT * FROM(SELECT PB.MAPB "Mã phòng ban", MONTH(NV.NGAYSINH) THANG,NV.MANV
...
BEGIN
    SELECT NV.MANV "Mã nhân viên",
           ...
           (dbo.FUNCTION_GET_TENHOPDONG(NV.MANV)) "Hợp đồng mới nhất",
           (dbo.FUNTION_GET_THOIGIAN_HOPDONG_MOINHAT(NV.MANV)) "Thời gian bắt đầu",
           (dbo.FUNCTION_GET_TENHOPDONG_LIENTRUOC(NV.MANV)) "Hợp đồng gần đây",
...
/**
	Lấy ra tên hợp đồng mới nhất theo mã nhân viên
*/
ALTER FUNCTION dbo.FUNCTION_GET_TENHOPDONG
(
    @MANV NVARCHAR(10)
)
...
BEGIN
    ...
    SELECT [Mã nhân viên],
           ...
           [Ngày bắt đầu],
           [Ngày kết thúc]
    FROM dbo.FUN_THONGTIN_HOPDONG_NHANVIEN();
    RETURN
    (
        SELECT TENHOPDONG "Tên hd" FROM @TEMP_TABLE WHERE MANV = @MANV
    );
END;
GO
...
*/
CREATE FUNCTION FUNCTION_GET_THOIGIAN_HOPDONG_LIENTRUOC
(
    @MANV NVARCHAR(10)
)
RETURNS DATETIME
    ...
    (
        ...
        (
            ...
            WHERE NV.MANV = HD.MANV
                  ...
                  AND HD.NGAYBATDAU <
                  (
                      SELECT dbo.FUNTION_GET_THOIGIAN_HOPDONG_MOINHAT(@MANV)
                  )
        ) AS TTHD
    );
END;
...
/**
	Lấy ra tên của hợp đồng liền trước hợp đồng mới nhất theo mã nhân viên
*/
CREATE FUNCTION FUNCTION_GET_TENHOPDONG_LIENTRUOC
(
    @MANV NVARCHAR(10)
)
...
BEGIN
    RETURN
    (
        SELECT TENLOAIHOPDONG FROM dbo.HOPDONG WHERE MANV = @MANV AND NGAYBATDAU = (SELECT dbo.FUNCTION_GET_THOIGIAN_HOPDONG_LIENTRUOC(@MANV))
    );
END;
GO

/** 
	Lấy ra thời gian của hợp đồng mới nhất theo mã nhân viên
*/
CREATE FUNCTION FUNTION_GET_THOIGIAN_HOPDONG_MOINHAT
(
    @MANV NVARCHAR(10)
)
RETURNS DATETIME
...
BEGIN
    ...
    (
        SELECT BATDAU FROM @TEMP_TABLE WHERE MANV = @MANV
    ));
END;
GO
