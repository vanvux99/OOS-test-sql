...
-- CAU 5:
ALTER FUNCTION HOPDONG_MOINHAT_FUNCTION
(
...
ALTER FUNCTION FUNCTION_GET_TENHOPDONG 
(
    @MANV NVARCHAR(10)
)
...
BEGIN
    ...
    SELECT *
    FROM dbo.HOPDONG_MOINHAT_FUNCTION(@MANV);
    RETURN
    (
        SELECT TENHOPDONG "Tên hd" FROM @TEMP_TABLE
    );
END;
GO
...
GO
SELECT * FROM
(SELECT MANV, HOVATEN, GIOITINH AS NAME, QUEQUAN FROM dbo.NHANVIEN) "NV"
PIVOT 
( 
	MAX(MANV) 
	FOR	NAME IN ([HOVATEN])
...
BEGIN
    SELECT NV.MANV "Mã nhân viên",
           ...
           (SELECT * FROM dbo.FUNCTION_GET_TENHOPDONG('NV1')) "Hợp đồng mới nhất",
           (dbo.THOIGIAN_HOPDONG_MOINHAT(NV.MANV)) "Thời gian bắt đầu",
		   (dbo.LIEN_TRUOC_HOP_DONG_MOI_NHAT_2(NV.MANV)) "Hợp đồng gần đây",
...
BEGIN
    ...
    (
        ...
        WHERE HD.NGAYBATDAU =
        (
            SELECT MAX(NGAYBATDAU) FROM dbo.HOPDONG WHERE HD.MANV = 'nv1'
        )
    );
END;
GO

CREATE FUNCTION FUNTION_HOPDONG_MOINHAT_CUA_NHANVIEN
(
    @MANV NVARCHAR(10)
)
RETURNS NVARCHAR(150)
...
SELECT DATEDIFF(s, GETDATE(), GETDATE())
