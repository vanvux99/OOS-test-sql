...
CREATE TABLE PHONGBAN (MAPB NVARCHAR(10) PRIMARY KEY, TENPB NVARCHAR(200));
GO

CREATE TABLE CHUCVU (MACV NVARCHAR(10) PRIMARY KEY, TENCV NVARCHAR(200));
GO
...
CREATE TABLE HOPDONG (MAHD NVARCHAR(10) PRIMARY KEY, MANV NVARCHAR(10), TENLOAIHOPDONG NVARCHAR(150), NGAYBATDAU DATETIME, NGAYKETTHUC DATETIME);
GO
...
CREATE PROCEDURE ADD_DATA_PHONGBAN (@MAPB NVARCHAR(10), @TENPB NVARCHAR(200))
AS
BEGIN
...
AS
BEGIN
    INSERT INTO dbo.NHANVIEN (MANV, HOVATEN, MAPB, MACV, NGAYSINH, GIOITINH, QUEQUAN, NGAYLAMVIEC)
    VALUES
...
CREATE PROCEDURE ADD_DATA_HOPDONG (@MAHD NVARCHAR(10), @MANV NVARCHAR(10), @TENHOPDONG NVARCHAR(150), @NGAYBATDAU DATETIME, @NGAYKETTHUC DATETIME)
AS
BEGIN
    INSERT INTO dbo.HOPDONG (MAHD, MANV, TENLOAIHOPDONG, NGAYBATDAU, NGAYKETTHUC)
    VALUES
...
/*
	Lấy thông tin nhân viên bao gồm các thông tin sau: Mã nhân viên, Họ và tên, Tên phòng ban, Tên chức vụ, ngày sinh
*/
CREATE PROC Proc_Get_ThongTinNhanVien
AS
BEGIN
    SELECT MANV,
           HOVATEN,
...
/*
	Lấy thông tin nhân viên của phòng ban có mã số bé nhất, thông tin cần: Mã nhân viên, Họ và tên, Mã phòng ban.
*/
CREATE PROC Proc_Get_MaPhongBanNhoNhat
AS
BEGIN
    SELECT NV.MAPB, NV.MANV, NV.HOVATEN FROM dbo.NHANVIEN NV WHERE NV.MAPB = (SELECT MIN(MAPB)FROM dbo.PHONGBAN);
END;
GO
...
/*
	Lấy thông tin phòng ban như sau: Mã phòng ban, Tên phòng ban, Số lượng nhân viên tại phòng ban đó
*/
CREATE PROC Proc_Get_ThongTinPhongBan
AS
BEGIN
    SELECT PB.MAPB, PB.TENPB, COUNT(PB.MAPB) EMPLOYEES_COUNT FROM dbo.NHANVIEN NV, dbo.PHONGBAN PB WHERE
    NV.MAPB = PB.MAPB GROUP BY PB.MAPB, PB.TENPB;
END;
GO

/*
	Lấy thông tin những chức vụ đang không có nhân viên nào đảm nhận, thông tin cần: Mã chức vụ, tên chức vụ.
*/
CREATE PROC Proc_Get_ChucVuKhongNhanVien
AS
BEGIN
    SELECT CV.MACV, CV.TENCV FROM dbo.CHUCVU CV INNER JOIN dbo.NHANVIEN NV ON CV.MACV = NV.MACV WHERE NV.MACV = '';
END;
GO
...
/*
	Tìm những nhân viên có quê ở Thái Bình và sinh sau năm 1990, thông tin cần: Mã nhân viên, Họ và tên, ngày sinh, quê quán.
*/
CREATE PROC Proc_Get_TimNhanVien
AS
BEGIN
...
-- b,
CREATE PROC PROC_Get_NhanVienTrungNgaySinh
AS
BEGIN
    SELECT MANV "Mã nhân viên", HOVATEN "Họ và tên", COUNT(NGAYSINH) "Ngày sinh bị trùng" FROM dbo.NHANVIEN GROUP BY
    MANV, HOVATEN HAVING COUNT(NGAYSINH) > 1;
END;
GO
...
RETURN SELECT HD.MANV,
       ...
       WHERE
           HD.NGAYBATDAU = (SELECT MAX(NGAYBATDAU)FROM dbo.HOPDONG WHERE dbo.HOPDONG.MANV = NV.MANV)
           AND NV.MANV = NV.MANV;
GO

-- CAU 6: 
-- a,
ALTER PROC dbo.TIENTO_PHONGBAN
AS
BEGIN
    ...
    SELECT NV.MANV "Mã nhân viên",
           ...
           (REPLACE(
            PB.TENPB,
            PB.TENPB,
            SUBSTRING(PB.TENPB, CHARINDEX('_', PB.TENPB) + 1, LEN(PB.TENPB) - CHARINDEX('_', PB.TENPB))
            )
           ) AS "Tên phòng ban"
    FROM
        dbo.NHANVIEN NV,
...
BEGIN
    ...
    FROM dbo.NHANVIEN
    WHERE
        MONTH(NGAYLAMVIEC) = @THANG
        AND YEAR(NGAYLAMVIEC) = @NAM;
END;
GO

-- b,
ALTER FUNCTION dbo.CONVERT_DATETIME (@DATE AS DATETIME)
RETURNS INT
AS
BEGIN
    RETURN (SELECT DATEDIFF(s, @DATE, GETDATE()));
END;
GO
...
BEGIN
    ...
    BEGIN
        DECLARE @DATE_GOTO_WORK INT =
            dbo.CONVERT_DATETIME(
                CONCAT_WS('-', @NAM, @THANG, DATEPART(DAY, DATEADD(DAY, -1, CONCAT_WS('-', @NAM, @THANG + 1, 1))))
            );
        SELECT MANV "Mã nhân viên",
               HOVATEN "Tên nhân viên",
               MAPB "Mã phòng ban",
...
BEGIN
    SELECT PVT.MAPB,
           ...
           PVT.[2],
           PVT.[1]
    FROM (SELECT PB.MAPB, MONTH(NV.NGAYSINH) THANG, NV.MANV FROM
          dbo.NHANVIEN NV INNER JOIN dbo.PHONGBAN PB ON PB.MAPB = NV.MAPB
    ) NVPB
    PIVOT (COUNT(MANV)
...
BEGIN
    DECLARE @month NVARCHAR(MAX),
            @query NVARCHAR(MAX);
    SELECT @month = STUFF((
        SELECT ',' + QUOTENAME(snnv.NGAYSINH) t1
    ...
    RETURN (
        ...
        FROM (
            ...
            WHERE
                NV.MANV = HD.MANV
                AND HD.MANV = @MANV
                AND HD.NGAYBATDAU < (SELECT dbo.FUNTION_GET_THOIGIAN_HOPDONG_MOINHAT(@MANV))
        ) AS TTHD
    );
END;
GO
...
BEGIN
    ...
    IF @ID = 1
    BEGIN
        SET @RESULT = (SELECT tb.TENLOAIHOPDONG FROM dbo.FUN_THONGTIN_HOPDONG_NHANVIEN() tb WHERE tb.MANV = @MANV);
    END;
    IF @ID = 2
    BEGIN
        SET @RESULT =
        (   SELECT TENLOAIHOPDONG FROM dbo.HOPDONG WHERE
            MANV = @MANV AND NGAYBATDAU = (SELECT dbo.FUNCTION_GET_THOIGIAN_HOPDONG_LIENTRUOC(@MANV))
        );
    END;
    RETURN @RESULT;
END;
...
BEGIN
    ...
    WHILE @INDEX <= DATEPART(DAY, DATEADD(DAY, -1, CONCAT_WS('-', @NAM, @THANG + 1, 1)))
    BEGIN
        INSERT INTO @TBL (NGAY, THU)
        VALUES
             (CONCAT_WS('-', @NAM, @THANG, @INDEX),                   -- NGAY - date
